...
<body>
    <h1>HazMat A&O Chapter {chapter} Practice</h1>
    <form id="quizForm">
'''
...
def strip_existing_header(html):
    """Removes everything up to and including <form id="quizForm"> or <body> if no form found."""
...
def strip_existing_footer(html):
    """Removes everything from </form> or <div id="results"> or <script> to the end."""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python add_header.py <html_file>")
        sys.exit(1)
    filename = sys.argv[1]
    ...
    # Compose new html
    new_html = HEADER_TEMPLATE.format(chapter=chapter) + body + FOOTER_TEMPLATE.format(chapter=chapter)
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(new_html)
    print(f"Header and footer added to {filename} (Chapter {chapter})")
